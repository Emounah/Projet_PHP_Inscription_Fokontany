...
for (let i = 0; i < optionsList.length; i++) {
    optionsList[i].addEventListener('click',()=>{
        ...
        document.querySelector('#Id-District-Commune').value = optionsList[i].id;
        document.querySelector('#NomRegion').value = optionsList[i].children[0].textContent;
        document.querySelector('#NomDistrict').value = optionsList[i].children[1].textContent;
        document.querySelector('#NomCommune').value = optionsList[i].children[2].textContent;
        selectBox.classList.remove('active');
    })
}
